/****************************************************************************
** Meta object code from reading C++ file 'startlauncherwindow.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.2.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../VRML-Browser/startlauncherwindow.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'startlauncherwindow.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.2.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_StartLauncherWindow_t {
    QByteArrayData data[9];
    char stringdata[131];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_StartLauncherWindow_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_StartLauncherWindow_t qt_meta_stringdata_StartLauncherWindow = {
    {
QT_MOC_LITERAL(0, 0, 19),
QT_MOC_LITERAL(1, 20, 20),
QT_MOC_LITERAL(2, 41, 0),
QT_MOC_LITERAL(3, 42, 20),
QT_MOC_LITERAL(4, 63, 19),
QT_MOC_LITERAL(5, 83, 11),
QT_MOC_LITERAL(6, 95, 8),
QT_MOC_LITERAL(7, 104, 12),
QT_MOC_LITERAL(8, 117, 12)
    },
    "StartLauncherWindow\0slotOpenSelectWindow\0"
    "\0slotOpenAboutMessage\0slotOpenKeysMessage\0"
    "slotGetPath\0filePath\0slotOpenFile\0"
    "slotEditFile\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_StartLauncherWindow[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   44,    2, 0x08,
       3,    0,   45,    2, 0x08,
       4,    0,   46,    2, 0x08,
       5,    1,   47,    2, 0x08,
       7,    0,   50,    2, 0x08,
       8,    0,   51,    2, 0x08,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString,    6,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void StartLauncherWindow::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        StartLauncherWindow *_t = static_cast<StartLauncherWindow *>(_o);
        switch (_id) {
        case 0: _t->slotOpenSelectWindow(); break;
        case 1: _t->slotOpenAboutMessage(); break;
        case 2: _t->slotOpenKeysMessage(); break;
        case 3: _t->slotGetPath((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 4: _t->slotOpenFile(); break;
        case 5: _t->slotEditFile(); break;
        default: ;
        }
    }
}

const QMetaObject StartLauncherWindow::staticMetaObject = {
    { &QMainWindow::staticMetaObject, qt_meta_stringdata_StartLauncherWindow.data,
      qt_meta_data_StartLauncherWindow,  qt_static_metacall, 0, 0}
};


const QMetaObject *StartLauncherWindow::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *StartLauncherWindow::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_StartLauncherWindow.stringdata))
        return static_cast<void*>(const_cast< StartLauncherWindow*>(this));
    return QMainWindow::qt_metacast(_clname);
}

int StartLauncherWindow::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 6)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 6;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 6)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
